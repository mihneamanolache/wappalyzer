...
for the entries in `scripts/lib/dnb-technologies.js`:
...
`test/dnb-technologies.test.js` asserts this directly: a paragraph naming nine
...
`scripts/lib/dnb-technologies.js`). They have full metadata and no detection
...
npm run coverage          # request-list coverage report
npm run coverage -- --markdown > docs/coverage.md
```
...
Put the definitions in `scripts/lib/dnb-technologies.js` and run:
...
`test/dnb-technologies.test.js` — including a negative case proving the pattern
does not over-match.
