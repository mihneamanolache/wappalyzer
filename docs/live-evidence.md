...
| | |
| --- | --- |
| Corpus | `data/dns-sweep-domains.txt` — 536 domains, committed |
...
`scripts/lib/dnb-technologies.js`, which stayed wrong after the sweep became
...
pattern. `test/catalog.test.js` asserts both invariants against the committed
catalog.
...
`scripts/xhr-audit.js` measures it against a committed list
