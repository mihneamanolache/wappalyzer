 ...
 * carrying the caveat.
 *
 * The corpus and results are not tracked, so the tests that read them skip on a
 * clone with a stated reason.
 *
...
// The coverage-report tests need the per-product inputs; the rest need the
// retained audit measurement. Both are untracked, so guard each independently.
const needsRequestData = skipUnless(hasRequestData, REQUEST_SKIP)
const needsAudit = skipUnless(hasAuditData, AUDIT_SKIP)

test('the audit url list is well-formed', needsAudit, () => {
...
test('the retained measurement is internally consistent', needsAudit, () => {
...
test('the measurement reflects a real scan, not a failed one', needsAudit, () => {
...
test('the coverage report carries the observability caveat', needsRequestData, () => {
...
test('the markdown report states the caveat in words', needsRequestData, () => {
...
test('xhr-only entries are still counted as catalog-matchable', needsRequestData, () => {
