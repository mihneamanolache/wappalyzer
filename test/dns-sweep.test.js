 ...
 * unverifiable. These tests assert that the retained results cover the markers
 * actually in the catalog and that no added marker is unobserved.
 *
 * The corpus and results are bulky measurements and are not tracked, so the tests
 * that read them skip on a clone with a stated reason. The ones that only check
 * catalog metadata always run.
 *
...
const {
    ...
    hasSweepData,
    skipUnless,
} = require('../scripts/lib/untracked')
const { TXT, TXT_ENRICH } = require('../scripts/lib/emerging-technologies')

const ROOT = path.resolve(__dirname, '..')
// The corpus and its retained results are untracked, so a clone lacks them.
// Note this covers more than the tests that read the files directly: EVIDENCE
// labels in emerging-technologies.js are derived from the retained sweep at load
// time, so assertions about those labels need it too. Tests of pure supersession
// logic still run.
const needsSweep = skipUnless(hasSweepData, SWEEP_SKIP)

const CORPUS = path.join(ROOT, 'data/dns-sweep-domains.txt')
const RESULTS = path.join(ROOT, 'data/dns-sweep-results.json')

test('the sweep corpus is well-formed', needsSweep, () => {
...
test('the retained results describe the corpus that produced them', needsSweep, () => {
...
test('every added marker has retained evidence', needsSweep, () => {
    ...
    const { EVIDENCE } = require('../scripts/lib/emerging-technologies')
...
test('the exact marker strings still in the catalog are the ones swept', needsSweep, () => {
...
test('the retained results cover the markers currently in the catalog', needsSweep, () => {
...
test('observed markers carry a domain count and examples', needsSweep, () => {
    const results = JSON.parse(fs.readFileSync(RESULTS, 'utf8'))
...
test('no marker is recorded as both observed and unobserved', needsSweep, () => {
    const results = JSON.parse(fs.readFileSync(RESULTS, 'utf8'))
...
test('the retained sweep carries auditable provenance', needsSweep, () => {
...
test('the corpus digest covers the domains, not the comments around them', needsSweep, () => {
    // The results depend on which domains were resolved. Hashing the raw file
    // meant that editing a comment invalidated retained evidence, which pushes
    // whoever hits it towards hand-editing the digest — the one thing a
    // provenance field must never invite. Hashing the parsed set instead keeps a
    // domain change fatal and a prose change harmless.
    const results = JSON.parse(fs.readFileSync(RESULTS, 'utf8'))
    ...
    assert.equal(
        results.corpusSha256,
        digest([...domains].sort().join('\n')),
        'retained digest should be the sha256 of the sorted parsed domain set'
    )

    // A comment must not change it; a domain must.
    assert.notEqual(
        results.corpusSha256,
        digest([...domains, 'added.example'].sort().join('\n')),
        'adding a domain must change the digest'
    )
})

test('every corpus domain is accounted for by a resolution outcome', needsSweep, () => {
...
test('the retained sweep matches the corpus and catalog on disk', needsSweep, () => {
...
test('supersession requires literal containment, not a shared name', () => {
    ...
    const { supersedes } = require('../scripts/lib/emerging-technologies')
...
test('runtime supersession is gated by the declared map', () => {
    ...
    const { supersedes, EXPECTED_SUPERSESSIONS } = require(
        '../scripts/lib/emerging-technologies'
    )
...
test('the five superseded markers are exactly the expected ones', needsSweep, () => {
    const {
        EVIDENCE,
...
test('the retained provenance digests are full SHA-256', needsSweep, () => {
