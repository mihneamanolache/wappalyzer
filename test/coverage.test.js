...
// A clone legitimately lacks the per-product inputs, so skip rather than fail —
// and name the reason, because a silently absent suite is worse than a loud one.
if (!hasRequestData()) {
    test('request-to-catalog mapping', { skip: REQUEST_SKIP }, () => {})

    return
}

const { classify, normalize } = require('../scripts/lib/requested-mapping')
 ...
 * a detection test in test/emerging-technologies.test.js.
 */
const UNDETECTABLE = {
