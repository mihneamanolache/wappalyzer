 ...
 *
 * Usage:
 *   node scripts/catalog-delta.js              # write docs/catalog-delta.{json,md}
...
const { CATALOG_ONLY, TECHNOLOGIES } = require('./lib/emerging-technologies')

const ROOT = path.resolve(__dirname, '..')
const OUT_JSON = path.join(ROOT, 'docs/catalog-delta.json')
const OUT_MD = path.join(ROOT, 'docs/catalog-delta.md')

/**
 * Is the recorded base commit present in this clone?
 *
 * CI checkouts are shallow by default, so `git show <base>:file` fails there for
 * reasons that have nothing to do with the delta being stale. Distinguish the two
 * so a shallow clone reports honestly instead of looking like a stale artifact.
 */
function baseIsReachable(base) {
    const result = spawnSync('git', ['cat-file', '-e', `${base}^{commit}`], {
        cwd: ROOT,
        stdio: 'ignore',
    })
...
function main() {
    ...
    // A shallow clone cannot resolve the recorded base, which is a different
    // problem from the artifact being stale. Say which one it is.
    if (!baseIsReachable(base)) {
        console.log(
            `Base commit ${base} is not present in this clone, so the delta ` +
                'cannot be verified.\n' +
                'Fetch full history (actions/checkout with fetch-depth: 0) to ' +
                'check it.'
        )
    ...
    if (CHECK_ONLY) {
        ...
        console.log(
            `docs/catalog-delta.{json,md} are current ` +
    ...
    console.log(
        `Wrote docs/catalog-delta.{json,md}\n` +
