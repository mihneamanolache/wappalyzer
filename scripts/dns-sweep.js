...
async function sweep() {
    ...
    const results = {
        ...
        corpusSha256: sha256(fs.readFileSync(CORPUS)),
...
async function main() {
    if (CHECK_ONLY) {
        if (!fs.existsSync(RESULTS)) {
            console.error(`No retained results at ${path.relative(ROOT, RESULTS)}`)
            process.exit(1)
        }
        ...
        const corpusNow = sha256(fs.readFileSync(CORPUS))
